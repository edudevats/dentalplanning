...
class CorreccionDiaSchema(Schema):
    """Corrección de la caja del día. La fecha NO se recibe: siempre es hoy."""

    class Meta:
        unknown = EXCLUDE

    # La caja que se está corrigiendo (origen).
    sucursal_id = fields.Int(allow_none=True, load_default=None)
    ...
    # A dónde se mueve la caja. Ausente o igual al origen = no se mueve.
    sucursal_destino_id = fields.Int(allow_none=True, load_default=None)
